...
1. **デモ View を設計する**
   - その文法要素だけを示す最小限の View にする（ただし、背景色など視認性を高めるためのコードの追加は容認する）
   - `@State` など状態管理が必要な場合 → 別 struct（`XxxDemoView`）として定義
   - 状態管理が不要な場合 → `AnyView(...)` にインラインで記述
   - **重要**: `contentView` の `AnyView(...)` に渡す View と、`code` 文字列の実装はほぼ同じにする。
     つまり `code` に書くコード例がそのまま `contentView` のプレビューとして動作するような設計にする。
...
4. **code 文字列を書く**: `import SwiftUI` から始め、デモ View の実装を記述する。
   `contentView` に渡す View と同じ struct・同じロジックを使い、差分が生じないようにする。
...
// contentView と code は同じ View 実装を使う。
// State なし（インライン）の場合:
TabData(
    title: "...",
    description: "...",
...
State ありで別 struct が必要な場合は、`contentView` には struct のインスタンスを渡し、`code` にも同じ struct 定義を記述する：
...
// TabData エントリ（contentView と code が対応）
TabData(
    title: "...",
    description: "...",
    contentView: AnyView(XxxDemoView()),   // ← 同じ struct を使う
    code: """
        import SwiftUI

        struct XxxDemoView: View {         // ← contentView と同じ実装
