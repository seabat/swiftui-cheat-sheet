...
struct TabBarScreen: View {
    ...
    var body: some View {
        ZStack(alignment: .bottomTrailing) {
            VStack(
                spacing: 0 // デフォルトの VStack アイテム間のスペース(8pt)を削除する
            ) {
...
private let sampleTabs: [TabData] = [
    TabData(
        title: "文字列を表示",
        description: "文字列を表示。.font に .largeTitle、 背景色に .black を指定。",
        contentView: AnyView(
            Text("文字列が表示されます。")
                .font(.largeTitle)
    ...
    TabData(
        title: "システム画像を表示",
        description: "システム画像 photo を表示。 画像を拡大。背景色に .blue を指定。",
        contentView: AnyView(
            VStack {
    ...
    TabData(
        ...
        description: "システム画像 star を表示。画像を拡大。",
        contentView: AnyView(
            VStack {
